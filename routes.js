...
router.get('/puzzle/:puzzle_id', (req, res) => {
  const puzzleToLookUp = req.params.puzzle_id; // print them out to confirm their contents:

  db.all( //SQL query:
  'SELECT * FROM puzzles WHERE puzzle_id=$id', {
    $id: puzzleToLookUp
  }, (err, row) => {
    //Callback function, runs when query finishes
    //     console.log(row)
    ...
    'SELECT user_id FROM users WHERE user_name=$name AND experience=$exp', {
      ...
      if (row.length > 0) {
        ...
        db.all('SELECT DISTINCT puzzle_id FROM user_moves WHERE user_id=$id', {
          ...
          }); // Return the last user with the given username (Row length should be 1) and the puzzles it has already tried/solved
        });
      } else {
        ...
        db.run(insertQuery, function (err) {
          if (err) {
            ...
            res.send({
              user_id: this.lastID
            }); // Sent back the new userid created
          }
        });
      }
    });
...
router.post('/logging', (req, res) => {
  let insertQuery = "INSERT INTO user_moves VALUES (" + req.body.uid + "," + req.body.pid + "," + req.body.q + "," + req.body.r + "," + req.body.s + "," + req.body.c + "," + req.body.moveNumber + "," + req.body.moveTime + "," + req.body.solved + ")"; // console.log(insertQuery);
...
router.post('/lograting', (req, res) => {
  let insertQuery = "INSERT INTO puzzle_rating VALUES (" + req.body.uid + "," + req.body.pid + "," + req.body.dif + "," + req.body.int + ")"; // console.log(insertQuery);
