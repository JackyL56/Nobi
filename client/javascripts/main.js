...
$(document).ready(() => {
  let userid = 0;
  let currentPuzzle;
  let sample_puzzle1 = 1;
  let sample_puzzle2 = 2; // Id's of the sample puzzles

  let numberOfSamplePuzzles = 2;
  let puzzle_min = 3,
      puzzle_max = 1102; // max - Puzzle id of the last puzzle in the database
  ...
  let getRandomPuzzle = function getRandomPuzzle() {
  ...
  $('#btnStart').click(function () {
    if (formcheck()) {
      ...
      let user = $('#username').val(); // Get Age variable
  ...
  $('#btnNextYes').click(function () {
    ...
    $('#gameCanvas').empty();
    currentPuzzle = getRandomPuzzle(puzzle_done); // game = new Nobi(userid,currentPuzzle);
